...
const LoadingState: React.FC = () => {
    const messages = useMemo(() => [
        ...
        "Identifying your soul planet...",
        "Revealing Jaimini insights...",
    ], []);
    ...
    return (
        <div className="flex flex-col items-center justify-center h-full p-8 text-center min-h-[60vh]">
            ...
            <h1 className="font-serif text-3xl font-bold text-brand-text-primary mt-8">Finding Your Karaka Planets</h1>
...
const Report: React.FC<{ data: KundliData, onReset: () => void }> = ({ data, onReset }) => {
    const { atmakaraka, darakaraka } = data.jaiminiAnalysis;
    return (
        <div className="max-w-2xl mx-auto mt-6 animate-fade-in space-y-6">
             <div className="bg-brand-card p-6 rounded-xl shadow-md">
                 <div className="flex items-center gap-4 mb-3">
                    <UserIcon className="w-8 h-8 text-brand-primary"/>
                    <h2 className="font-serif text-2xl font-bold text-brand-primary">Atmakaraka: {atmakaraka.planet}</h2>
                </div>
                 <p className="text-brand-text-secondary leading-relaxed">{atmakaraka.description}</p>
            </div>
             <div className="bg-brand-card p-6 rounded-xl shadow-md">
                 <div className="flex items-center gap-4 mb-3">
                    <HeartIcon className="w-8 h-8 text-brand-red"/>
                    <h2 className="font-serif text-2xl font-bold text-brand-red">Darakaraka: {darakaraka.planet}</h2>
                </div>
                 <p className="text-brand-text-secondary leading-relaxed">{darakaraka.description}</p>
            </div>
...
export const AtmakarakaCalculatorPage: React.FC = () => {
    ...
    return (
        <div>
            <header className="mb-6 text-center">
                <h1 className="font-serif text-3xl sm:text-4xl font-bold text-brand-text-primary">Atmakaraka & Darakaraka Calculator</h1>
                <p className="text-brand-text-secondary mt-1 max-w-lg mx-auto">Discover your Jaimini Karakas: the Atmakaraka (planet of the self) and the Darakaraka (planet of the spouse) to understand your soul's purpose and relationships.</p>
            </header>
